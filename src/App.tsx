        ...
        <div className="contact-grid">
          {c.contact.items.map(({ icon, label, value, href }) => (
            <a key={icon} href={href} target={icon === 'github' || icon === 'linkedin' ? '_blank' : undefined} rel="noreferrer" className="glass-card contact-card">
              <span className={`contact-card-icon${icon === 'github' || icon === 'linkedin' ? ' contact-card-icon--brand' : ''}`}>
                ...
                {icon === 'github'   && <img src={githubLogo}   alt="" style={{ width: 22, height: 22 }} />}
                {icon === 'linkedin' && <img src={linkedinLogo} alt="" style={{ width: 22, height: 22 }} />}
              </span>
